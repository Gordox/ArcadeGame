...
namespace HeroSiege.AISystems.PathFinding
{
    /// <summary>
    ...
    public class SearchNode
    {
        ...
        /// <summary>
        /// Priority over tile, less means higher priority
        /// </summary>
    ...
    class Pathfinder
    {
        ...
        /// Constructor.
        /// </summary>
        public Pathfinder(TileMap map, int width, int height)
        {
            levelWidth = width;
            levelHeight = height;
        ...
        private void InitializeSearchNodes(TileMap map)
        {
            ...
            for (int x = 0; x < levelWidth; x++)
            {
                for (int y = 0; y < levelHeight; y++)
                {
                    ...
                    // Our units can only walk on road tiles.
                    //make this a switch state to deturment if it is wakeble
                    node.Walkable = map.GetIfWakeble(x, y);

                    switch (map.GetTileType(x, y))
                    {
                        case WalkTypes.Walkable:
                            node.Walkable = true;
                            break;
                        case WalkTypes.Grass:
                            node.Walkable = true;
                            break;
                        case WalkTypes.Obstacle:
                            node.Walkable = false;
                            break;
                        case WalkTypes.None:
                            break;
        ...
        private void GetMood(TileMap map, int x, int y, SearchNode node)
        {
            if (map.GetTileType(x, y) == WalkTypes.Walkable)
            {
                node.priority = 1f;
            }
        ...
        private Queue<Vector2> FindFinalPath(SearchNode startNode, SearchNode endNode)
        {
            ...
            for (int i = closedList.Count - 1; i >= 0; i--)
            {
                /*
                 * IMPORTANT!!!!!
                 * '32' and '16' needed to change dependent on how big the path squareds are!
                 */
                finalPath.Enqueue(new Vector2(closedList[i].Position.X * 32 + 16,
        ...
        public Queue<Vector2> FindPath(Point startPoint, Point endPoint)
        {
            ...
            // Hanterar ett error, möjligvis inte den bästa lösningen
            if (startNode == null)
                return new Queue<Vector2>();
