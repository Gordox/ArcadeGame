...
{
    class LightningAxe : Projectile
    {
        const float LIFE_TIME = 1.5f; //1.5 sec
        const int DAMAGE = 20;

        public LightningAxe(string animationName, FrameAnimation animation, float x, float y, float width, float height, Entity target, int dmg = 0)
            : base(animationName, animation, x, y, width, height, target, dmg)
        {
        }
        public LightningAxe(string animationName, FrameAnimation animation, float x, float y, float width, float height, Direction direction, int dmg = 0)
